...
class Model(nn.Module):
    def __init__(self, block, num_blocks, num_input_images=1, num_classes=1000, stem=False):
        super(Model, self).__init__()
        self.in_places = 64

        self.num_ch_enc = np.array([64, 64, 128, 256, 512]) # for ResNet18
    ...
    def forward(self, input_image):
        ...
        x = self.conv1(x)
        x = self.bn1(x)
        self.features.append(self.relu1(x))
        self.features.append(self.layer1(self.maxpool1(self.features[-1])))
...
def ResNet18(num_classes=1000, num_input_images=1, stem=False):
    return Model(Bottleneck, [2, 2, 2, 2], num_input_images=num_input_images, stem=stem)
